...
import io.tensevntysevn.ota.download.DownloadRom;
import io.tensevntysevn.ota.download.DownloadRomProgress;
import io.tensevntysevn.ota.tasks.GenerateRecoveryScript;
...
public class AvailableActivity extends Activity implements Constants, android.view.View
    ...
    public static void setupProgress(Context context) {
        ...
        boolean downloadFinished = TnsOtaPreferences.getDownloadFinished(context);
        if (downloadFinished) {
            if (DEBUGGING)
            ...
            int color;
            if (TnsOtaPreferences.getCurrentTheme(context) == 0) { // Light
            ...
            String fileSizeStr = TnsOtaUtils.formatDataFromBytes(fileSize);
            if (mProgressCounterText != null) {
    ...
    public static void updateProgress(int progress, int downloaded, int total) {
        ...
        mProgressCounterText.setText(
                TnsOtaUtils.formatDataFromBytes(downloaded) +
                        "/" +
                        TnsOtaUtils.formatDataFromBytes(total));
    }
    ...
    public static void setupMenuToolbar(Context context) {
        ...
        boolean md5HasRun = TnsOtaPreferences.getHasMD5Run(context);
        boolean md5Passed = TnsOtaPreferences.getMD5Passed(context);
        ...
        if (!downloadFinished) { // Download hasn't finished
            ...
            String md5 = OtaManifestUtils.getMd5(context);
            if (!md5.equals("null")) {
                // Is MD5 being used?
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        if (TnsOtaPreferences.getIsDownloadOnGoing(mContext)) {
    ...
    public boolean onOptionsItemSelected(MenuItem item) {
        ...
        switch (item.getItemId()) {
            case R.id.menu_available_check_md5:
                ...
                TnsOtaPreferences.setHasMD5Run(mContext, true);
                return true;
    ...
    public boolean onPrepareOptionsMenu(Menu menu) {
        ...
        boolean downloadFinished = TnsOtaPreferences.getDownloadFinished(mContext);
        boolean downloadIsRunning = TnsOtaPreferences.getIsDownloadOnGoing(mContext);
        boolean md5HasRun = TnsOtaPreferences.getHasMD5Run(mContext);
        boolean md5Passed = TnsOtaPreferences.getMD5Passed(mContext);
        ...
        if (!downloadFinished) { // Download hasn't finished
            ...
            String md5 = OtaManifestUtils.getMd5(mContext);
            if (!md5.equals("null")) {
                // Is MD5 being used?
                if (md5HasRun && md5Passed) {
    ...
    private void setupDialogs() {
        ...
        mDeleteDialog.setTitle(R.string.are_you_sure)
                ...
                .setPositiveButton(R.string.ok, (dialog, which) -> {
                    ...
                    TnsOtaUtils.deleteFile(OtaManifestUtils.getFullFile(mContext)); // Delete the file
                    TnsOtaPreferences.setHasMD5Run(mContext, false); // MD5 check hasn't been run
                    TnsOtaPreferences.setDownloadFinished(mContext, false);
        ...
        mRebootDialog.setTitle(R.string.are_you_sure)
                ...
                .setPositiveButton(R.string.ok, (dialog, which) -> {
                    if (DEBUGGING) Log.d(TAG, "ORS is " + TnsOtaPreferences.getORSEnabled(mContext));

                    TnsOtaPreferences.setDeviceUpdating(context, true);

                    if (TnsOtaPreferences.getORSEnabled(mContext)) {
    ...
    private void setupUpdateNameInfo() {
        boolean isDownloadOnGoing = TnsOtaPreferences.getIsDownloadOnGoing(mContext);
        ...
        String filename = OtaManifestUtils.getFilename(mContext);
    ...
    private void download() {
        String httpUrl = OtaManifestUtils.getHttpUrl(mContext);
        String directUrl = OtaManifestUtils.getDirectUrl(mContext);
        String error = getResources().getString(R.string.available_url_error);

        boolean isMobile = TnsOtaUtils.isMobileNetwork(mContext);
        boolean isSettingWiFiOnly = TnsOtaPreferences.getNetworkType(mContext).equals(WIFI_ONLY);
    ...
    private class MD5Check extends AsyncTask<Object, Boolean, Boolean> {
        ...
        protected Boolean doInBackground(Object... params) {
            String file = OtaManifestUtils.getFullFile(mContext).getAbsolutePath(); // Full file, with path
            String md5Remote = OtaManifestUtils.getMd5(mContext); // Remote MD5 form the manifest. This
        ...
        protected void onPostExecute(Boolean result) {
            ...
            TnsOtaPreferences.setMD5Passed(mContext, result); // Set value for other persistent settings
            setupMenuToolbar(mContext); // Reset options menu
