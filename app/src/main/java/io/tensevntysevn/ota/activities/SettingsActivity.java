...
import android.content.SharedPreferencesOnSharedPreferenceChangeListener;
...
public class SettingsActivity extends PreferenceActivity implements OnPreferenceClickListener,
    ...
    public void onCreate(Bundle savedInstanceState) {
        mContext = this;
        setTheme(TnsOtaPreferences.getSettingsTheme(mContext));
        super.onCreate(savedInstanceState);

        getPreferenceManager().setSharedPreferencesName(TnsOtaPreferences.PREF_NAME);
        PreferenceManager.setDefaultValues(this, R.xml.preferences, false);
        addTnsOtaPreferences.FromResource(R.xml.preferences);
        ...
        String ignoredRelease = TnsOtaPreferences.getIgnoredRelease(mContext);
    ...
    @Override
    public void onSharedPreferenceChanged(SharedPreferences sharedTnsOtaPreferences., String key) {
        ...
        if (pref instanceof ListPreference) {
            ...
            if (key.equals(CURRENT_THEME)) {
                TnsOtaPreferences.setTheme(mContext, listPref.getValue());
                this.recreate();
            } else if (key.equals(UPDATER_BACK_FREQ)) {
                TnsOtaUtils.setBackgroundCheck(mContext, TnsOtaPreferences.getBackgroundService(mContext));
            }
    ...
    private void setNotIgnore(boolean set) {
        if (set) TnsOtaPreferences.setIgnoredRelease(mContext, "0");
        mIgnoredRelease.setSummary(
    ...
    private void showInstallPrefs() {
        ...
        wipeData = TnsOtaPreferences.getWipeData(mContext);
        wipeCache = TnsOtaPreferences.getWipeCache(mContext);
        wipeDalvik = TnsOtaPreferences.getWipeDalvik(mContext);
        deleteAfterInstall = TnsOtaPreferences.getDeleteAfterInstall(mContext);
        ...
        mInstallPrefsDialog.setPositiveButton(R.string.ok, (dialog, id) -> {
            TnsOtaPreferences.setWipeData(mContext, mInstallPrefsItems.get(0));
            TnsOtaPreferences.setWipeCache(mContext, mInstallPrefsItems.get(1));
            TnsOtaPreferences.setWipeDalvik(mContext, mInstallPrefsItems.get(2));
            TnsOtaPreferences.setDeleteAfterInstall(mContext, mInstallPrefsItems.get(3));
        });
