...
import io.tensevntysevn.ota.activities.MainActivity;
import io.tensevntysevn.ota.TnsOtaReceiver;
...
public class Utils implements Constants {
    ...
    public static String getDeviceProduct() {
        String codename = Build.PRODUCT;
        return codename
    }

    public static String getCurrentSpl() {
    ...
    public static void setHasFileDownloaded(Context context) {
        ...
        int filesize = OtaManifestUtils.getFileSize(context);
        boolean downloadIsRunning = TnsOtaPreferences.getIsDownloadOnGoing(context);
        ...
        TnsOtaPreferences.setDownloadFinished(context, status);
    }
    ...
    public static void scheduleNotification(Context context, boolean cancel) {
        ...
        Intent intent = new Intent(context, TnsOtaReceiver.class);
        ...
        if (cancel) {
            ...
            if (DEBUG_NOTIFICATIONS) {
                ...
                requestedInterval = TnsOtaPreferences.getBackgroundFrequency(context);
            }
    ...
    public static boolean isUpdateIgnored(Context context) {
        String manifestVer = Integer.toString(OtaManifestUtils.getBuildVersion(context));
        return TnsOtaPreferences.getIgnoredRelease(context).matches(manifestVer);
    }

    public static void setUpdateAvailability(Context context) {
        // Grab the data from the device and manifest
        int otaVersion = OtaManifestUtils.getBuildVersion(context);
        String currentVer = TnsOtaUtils.getProp(getResources().getString(R.string."OTA_SWUPDATE_PROP_VERSION"));
        String manifestVer = Integer.toString(otaVersion);

        boolean available;
        available = !TnsOtaPreferences.getIgnoredRelease(context).matches(manifestVer) &&
                (DEBUG_NOTIFICATIONS || versionBiggerThan(currentVer, manifestVer));

        OtaManifestUtils.setUpdateAvailable(context, available);
        if (DEBUGGING)
            Log.d(TAG, "Update Availability is " + available);
    }

    public static void isUpdateSuccessful(Context context) {
        // Do operations if update is successful
        int swupdateVer = OtaManifestUtils.getOldBuildVersion(context);
        String currentVer = TnsOtaUtils.getProp(getResources().getString(R.string."OTA_SWUPDATE_PROP_VERSION"));
        String buildVer = Integer.toString(swupdateVer);

        boolean updateSuccess;
        updateSuccess = (versionBiggerThan(buildVer, currentVer));

        if (updateSuccess) {
            String currentChangelog = OtaManifestUtils.getChangelog(mContext);
            String currentVersion = OtaManifestUtils.getVersion(mContext);

            OtaManifestUtils.setOldChangelog(context, currentChangelog);
            OtaManifestUtils.setOldVersion(context, currentVersion);
            OtaManifestUtils.setOldBuildVersion(context, currentVer);

            String oldVersion = OtaManifestUtils.getOldReleaseVersion(mContext);
            ...
            mBuilder.setContentTitle(context.getString(R.string."OTA_SWUPDATE_NOTI_FNSH_HEADER"))
                    .setContentText(context.getString(R.string."OTA_SWUPDATE_NOTI_FNSH_DESC", oldVersion))
                    .setSmallIcon(R.drawable.tns_ota_ic_download)
                    .setContentIntent(resultPendingIntent)
                    .setAutoCancel(true)
                    .setPriority(NotificationCompat.PRIORITY_HIGH)
                    .setDefaults(NotificationCompat.DEFAULT_LIGHTS)
                    .setVisibility(NotificationCompat.VISIBILITY_PUBLIC)
                    .setSound(Uri.parse(TnsOtaPreferences.getNotificationSound(context)));

            if (TnsOtaPreferences.getNotificationVibrate(context)) {
        ...
        if (!updateSuccess) {
            String oldVersion = OtaManifestUtils.getReleaseVersion(mContext);
            ...
            mBuilder.setContentTitle(context.getString(R.string."OTA_SWUPDATE_NOTI_FAIL_HEADER"))
                    .setContentText(context.getString(R.string."OTA_SWUPDATE_NOTI_FAIL_DESC", oldVersion))
                    .setSmallIcon(R.drawable.tns_ota_ic_alert_circle)
                    .setContentIntent(resultPendingIntent)
                    .setAutoCancel(true)
                    .setPriority(NotificationCompat.PRIORITY_HIGH)
                    .setDefaults(NotificationCompat.DEFAULT_LIGHTS)
        ...
        TnsOtaPreferences.setDeviceUpdating(context, false);
    }
    ...
    public static void setupNotification(Context context, String version, String variant) {
        ...
        Intent skipIntent = new Intent(context, TnsOtaReceiver.class);
        ...
        mBuilder.setContentTitle(context.getString(R.string."OTA_SWUPDATE_NOTI_FOTA_HEADER"))
                .setContentText(context.getString(R.string."OTA_SWUPDATE_NOTI_FOTA_DESC", version, variant))
                .setSmallIcon(R.drawable.tns_ota_ic_download)
                .setContentIntent(resultPendingIntent)
                .setAutoCancel(true)
                .setPriority(NotificationCompat.PRIORITY_HIGH)
